...
# cursor-agent-bezel "<intent>" [--step-digest TEXT] -- <cursor-agent args>
# cursor-agent-bezel --catalog
# cursor-agent-bezel --schema TOOL
# `cursor-agent-jev` is the same program, for one transition.
    ...
    text = ''
      ...
      while [ "$#" -gt 0 ]; do
        ...
        if [ "$1" = "--schema" ] || [ "$1" = "--schema-dump" ]; then
          if [ -z "''${2:-}" ] || [ "$2" = "--" ]; then
            echo "cursor-agent-bezel: --schema needs a tool name" >&2
            exit 2
          fi
          schema_tool="$2"
          shift 2
      ...
      if ! command -v cursor-agent >/dev/null 2>&1; then
        echo "cursor-agent-bezel: cursor-agent not on PATH (fail closed)." >&2
        exit 127
      fi
      ...
      if [ "$mode" = "shadow" ]; then
        if [ "$router_rc" -ne 0 ]; then
          echo "cursor-agent-bezel: router rc=$router_rc; shadow does not block" >&2
        fi
        rm -f "$tmp" "$router_err"
        trap - EXIT
        exec cursor-agent "$@"
      ...
      if [ "$router_rc" -ne 0 ]; then
        echo "cursor-agent-bezel: active router rc=$router_rc choice=$choice gate=$gate — not execing cursor-agent" >&2
        exit 2
      fi

      if [ "$choice" = "stop" ] || [ "$choice" = "escalate" ]; then
        echo "cursor-agent-bezel: active choice=$choice gate=$gate hitl=$hitl — stop/escalate do not exec (HITL, no auto-retry)" >&2
        exit 2
      fi

      if [ "$blocked" = "true" ]; then
        echo "cursor-agent-bezel: active blocked=true choice=$choice gate=$gate — not execing cursor-agent" >&2
        exit 2
      fi

      if [ "$choice" != "continue" ] && [ "$gate" != "auto" ]; then
        echo "cursor-agent-bezel: active choice=$choice gate=$gate — honor continue/auto only" >&2
        exit 2
      fi
...
symlinkJoin {
  ...
  paths = [ inner ];
  postBuild = ''
    ln -s cursor-agent-bezel $out/bin/cursor-agent-jev
  '';
  meta = {
    description = "Bezel gate in front of cursor-agent. Shadow logs and does not block.";
    mainProgram = "cursor-agent-bezel";
